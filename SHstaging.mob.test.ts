...
test('test open article on category page', async ({ page }) => {
  await page.goto('https://frontend-stage.seoghoer.dk/kendte');
  await page.getByRole('button', { name: 'Allow all cookies' }).click();
  await page.locator('//div[contains(@class ,"max-w-full text-center bg-grey-200 lg:bg-white px-30 md:px-0 py-8 md:py-0 md:text-left")]').hover();
  await page.locator('//div[contains(@class ,"max-w-full text-center bg-grey-200 lg:bg-white px-30 md:px-0 py-8 md:py-0 md:text-left")]').click();
  await page.evaluate(() => { 
...
test('test newsletter sign-up', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Allow all cookies' }).click();
  await page.locator('//input[@placeholder="e-mail"]').fill('jjk@mailinator.com');
  await page.locator('//input[@id="fornavn"]').fill('Jens');
  await page.locator('//input[@id="efternavn"]').fill('jakob');
