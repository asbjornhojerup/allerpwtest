...
test('test go to frontpage by clicking logo', async ({ page }) => {
    await page.goto('https://frontend-stage.femina.dk/agenda');
    await page.getByRole('button', { name: 'Tillad alle cookies' }).click();
...
test('test open article on category page', async ({ page }) => {
    await page.goto('https://frontend-stage.femina.dk/agenda');
    await page.getByRole('button', { name: 'Tillad alle cookies' }).click();
    await page.locator('//*[@class="block group flex flex-col justify-center items-center h-full"]').first().click();
...
//test('test open article on frontpage', async ({ page }) => {
  ...
  //  await expect(page.locator('//*[@class="flex gap-x-10 mb-10"]')).toBeVisible();
//}); 

//tests if jw player is loaded after marketing consent is given
test('test video player, total consent', async ({ page }) => {
    await page.goto('https://frontend-stage.femina.dk/agenda/karriere-og-penge/kendt-radiovaert-erstatter-sofie-linde-som-x-factor-vaert');
    await page.getByRole('button', { name: 'Tillad alle cookies' }).click();
  await page.evaluate(() => { 
    window.scrollBy(0, 1000);
  });
...
test('newsletter sign-up', async ({ page }) => {
  await page.goto('https://frontend-stage.femina.dk/nyhedsbrev');
  await page.getByRole('button', { name: 'Tillad alle cookies' }).click();
...
test('sign in to plus', async ({ page }) => {
  await page.goto('https://frontend-stage.femina.dk/plus');
  await page.getByRole('button', { name: 'Tillad alle cookies' }).click();
...
test('test ad placements on article', async ({ page }) => {
  await page.goto('https://frontend-stage.femina.dk/sundhed/jeg-er-paa-vej-hjem-til-dig-nu-sagde-vagtlaegen-i-telefonen-ingen-havde-regnet-med-noget');
  await page.getByRole('button', { name: 'Tillad alle cookies' }).click();
  await expect(page.locator('#responsive_1-1')).toBeVisible();
});