"""Vielskerserier production tests - desktop"""
import pytest
...
def test_accept_all_cookies(page_desktop: Page):
    ...
    page_desktop.goto("https://vielskerserier.dk/")
...
def test_load_more_on_category_page(page_desktop: Page):
    ...
    page_desktop.goto("https://vielskerserier.dk/film/film-nyheder/")
    page_desktop.get_by_role("button", name="Tillad alle").click()
...
def test_direct_link_to_article(page_desktop: Page):
    ...
    page_desktop.goto(
        "https://vielskerserier.dk/film/film-nyheder/den-sidste-viking-udtaget-til-prestigefyldt-filmfestival"
    )
    page_desktop.get_by_role("button", name="Tillad alle").click()
...
def test_ad_placements_on_article(page_desktop: Page):
    ...
    page_desktop.goto(
        "https://vielskerserier.dk/film/film-anmeldelser/anmeldelse-cillian-murphy-kan-som-ingen-anden-fortaelle-en-hel-historie-med"
    )
    page_desktop.get_by_role("button", name="Tillad alle").click()
    expect(page_desktop.locator("#responsive_1-1")).to_be_visible()
