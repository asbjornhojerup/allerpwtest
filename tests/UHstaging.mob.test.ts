...
await page.locator('#header-main').getByRole('link', { name: 'Skæbner' }).first().click();
await expect(page.getByRole('heading', { name: 'Skæbner' })).toBeVisible();
});


test('test load more  on category page', async ({ page }) => {
await page.goto('https://frontend-stage.udeoghjemme.dk/skaebner');
await page.getByRole('button', { name: 'Allow all cookies' }).click();
...
await page.goto('https://frontend-stage.udeoghjemme.dk/nyhedsbrev');
await page.getByRole('button', { name: 'Allow all cookies' }).click();
...
await page.goto('https://frontend-stage.udeoghjemme.dk/reality/love-island/love-island-deltageren-jamie-eschen-skal-i-faengsel-igen-hun-kunne-ikke-faa');
await page.getByRole('button', { name: 'Allow all cookies' }).click();
});

//tests if jw player is loaded after marketing consent is given
test('test video player, total consent', async ({ page }) => {
await page.goto('https://frontend-stage.udeoghjemme.dk/skaebner/naar-livet-goer-ondt/marlene-fik-sin-elskede-john-viet-paa-hans-doedsleje');
await page.getByRole('button', { name: 'Allow all cookies' }).click();
await page.evaluate(() => { 
...
await page.goto('https://frontend-stage.udeoghjemme.dk/skaebner/naar-livet-goer-ondt/marlene-fik-sin-elskede-john-viet-paa-hans-doedsleje?debugAds=true');
await page.getByRole('button', { name: 'Allow all cookies' }).click();
  await expect(page.locator('#responsive_1-1')).toBeVisible();
});