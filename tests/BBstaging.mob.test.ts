...
test('test go to frontpage by clicking logo', async ({ page }) => {
  await page.goto('https://frontend-stage.billedbladet.dk/kendte');
  await page.getByRole('button', { name: 'Allow all cookies' }).click();
...
test('test newsletter sign-up', async ({ page }) => {
  await page.goto('https://frontend-stage.billedbladet.dk/nyhedsbrev');
  await page.getByRole('button', { name: 'Allow all cookies' }).click();
...
test('direct link to article', async ({ page }) => {
  await page.goto('https://frontend-stage.billedbladet.dk/nostalgi/husker-du-da-benedikte-og-anne-marie-forstyrrede-fars-nytaarstale');
  await page.getByRole('button', { name: 'Allow all cookies' }).click();
});
...
test('test ad placements on frontpage with necessary consent', async ({ page }) => {
  await page.goto('https://frontend-stage.billedbladet.dk');
  await page.getByRole('button', { name: 'Use necessary cookies only' }).click();
  await expect(page.locator('#responsive_1-1')).toBeEnabled();
});
...
///  await page.evaluate(() => { 
  ...
  //});
   // await page.evaluate(() => { 
    //window.scrollBy(0, 2000);
  //});
  //await expect(page.getByText('mob_intext_ad_bottom')).toBeVisible();
//});