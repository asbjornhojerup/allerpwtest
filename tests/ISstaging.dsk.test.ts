...
test('test open menu item, go to category', async ({ page }) => {
  await page.goto('https://frontend-stage.isabellas.dk/');
  await page.getByRole('button', { name: 'Tillad alle cookies' }).click();
  await page.getByRole('link', { name: 'Have' }).first().hover();
...
test('test load more  on category page', async ({ page }) => {
  await page.goto('https://frontend-stage.isabellas.dk/haven');
  await page.getByRole('button', { name: 'Tillad alle cookies' }).click();
...
//test('test newsletter sign-up', async ({ page }) => {
  //await page.goto('https://frontend-stage.isabellas.dk/nyhedsbrev');
  //await page.getByRole('button', { name: 'Tillad alle cookies' }).click();
