...
def test_video_player_total_consent(page_mobile: Page):
    ...
    page_mobile.goto(
        "https://frontend-stage.femina.dk/agenda/karriere-og-penge/kendt-radiovaert-erstatter-sofie-linde-som-x-factor-vaert"
    )
    page_mobile.get_by_role("button", name="Tillad alle cookies").click()
    page_mobile.evaluate("window.scrollBy(0, 1000)")
...
def test_newsletter_signup(page_mobile: Page):
    ...
    page_mobile.goto("https://frontend-stage.femina.dk/nyhedsbrev")
    page_mobile.get_by_role("button", name="Tillad alle cookies").click()
...
def test_sign_in_to_plus(page_mobile: Page):
    ...
    page_mobile.goto("https://frontend-stage.femina.dk/plus")
    page_mobile.get_by_role("button", name="Tillad alle cookies").click()
...
def test_ad_placements_on_article(page_mobile: Page):
    ...
    page_mobile.goto(
        "https://frontend-stage.femina.dk/sundhed/jeg-er-paa-vej-hjem-til-dig-nu-sagde-vagtlaegen-i-telefonen-ingen-havde-regnet-med-noget"
    )
    page_mobile.get_by_role("button", name="Tillad alle cookies").click()
    expect(page_mobile.locator("#responsive_1-1")).to_be_visible()
