"""Seoghoer production tests - desktop"""
import pytest
...
def test_accept_all_cookies(page_desktop: Page):
    ...
    page_desktop.goto("https://seoghoer.dk/")
...
def test_open_menu_go_to_category(page_desktop: Page):
    ...
    page_desktop.get_by_role("link", name="Kendte", exact=True).click()


def test_go_to_frontpage_by_clicking_logo(page_desktop: Page):
    """Test returning to frontpage via logo."""
    page_desktop.goto("https://seoghoer.dk/kendte")
    page_desktop.get_by_role("button", name="Tillad alle").click()
...
def test_open_article_on_category_page(page_desktop: Page):
    ...
    page_desktop.goto("https://seoghoer.dk/kongelige")
    page_desktop.get_by_role("button", name="Tillad alle").click()
    page_desktop.locator('//div[contains(@class ,"relative mr-10 md:mr-0 md:mb-10 bg-grey-300 aspect-video w-full lg:min-w-[160px]")]').hover()
    page_desktop.locator('//div[contains(@class ,"relative mr-10 md:mr-0 md:mb-10 bg-grey-300 aspect-video w-full lg:min-w-[160px]")]').click()
...
def test_newsletter_signup(page_desktop: Page):
    ...
    page_desktop.goto("https://seoghoer.dk/nyhedsbrev")
    page_desktop.get_by_role("button", name="Tillad alle").click()
...
def test_ad_placements_on_frontpage(page_desktop: Page):
    ...
    page_desktop.goto("https://seoghoer.dk?debugAds=true")
    page_desktop.get_by_role("button", name="Tillad alle").click()
...
def test_ad_placements_on_article(page_desktop: Page):
    ...
    page_desktop.goto(
        "https://seoghoer.dk/kendte/midt-i-sygdomschok-kendte-sender-kaerlighed-til-melvin-kakooza?debugAds=true"
    )
    page_desktop.get_by_role("button", name="Tillad alle").click()
    expect(page_desktop.locator("#responsive_1-1")).to_be_visible()
